...
Plot pane width. Twice during the build, the chart rendered with no bars at all. The country labels, legend and axis text were all present, but the plotting area itself had collapsed, and in the earlier attempt the x-axis labels overlapped into an unreadable blob. Both times I assumed I had broken something in the code and went looking for the fault there. The actual cause was that my RStudio plot pane was too narrow. ggplot2 allocates horizontal space to the axis labels, legend and margins first and gives the panel whatever remains, so with a narrow pane there was almost nothing left for the bars to occupy. Dragging the divider to the left to widen the viewer fixed it immediately, with no change to the code. The lesson was to check the output device before debugging the code, and to rely on ggsave() with explicit width and height arguments so that the saved file does not depend on the size of the pane at all. 
![Error with viewing panel making my output look corrupted](meat-preferences-by-country.jpg)

Missing country. When I pulled the data from FAOSTAT I filtered for both 2022 and 2023 without registering what that did to the country list. Every country I kept had values in both years, but Japan only had a 2022 value, so the joint filter silently dropped it. I did not notice until after I had already substituted South Korea in its place, and rather than redownload the data and rebuild the pipeline I decided to proceed with South Korea. The reproduction therefore differs from the original in one country, and the underlying lesson was to check the row count against the expected list of countries immediately after filtering, rather than assuming a filter has only done what I intended. 

Overall, I'm very happy with my recreation of the visualisation, I think it looked almost identical to the original source except for a few spacing differences and the fact that mine shows a dedicated Legend displaying the colours corresponding to each meat type while the original didn't have a legend, it has captions inside the chart itself that sits on-top of each protein option, some with arrows pointing to it. I did not know how to recreate that exactly yet, but I'm still proud of my progress and my final rendition of the Masters work! 
